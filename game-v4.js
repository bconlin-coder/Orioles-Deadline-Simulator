...
const bucketIds = {
  starter: ["joe-ryan", "logan-webb", "sandy-alcantara", "reid-detmers", "michael-wacha"],
  bullpen: ["adrian-morejon", "luke-weaver", "kenley-jansen", "ryan-zeferjahn", "brock-burke", "daniel-lynch", "steven-okert"],
  bat: ["luis-arraez", "jung-hoo-lee", "isaac-paredes", "mickey-moniak"],
  seller: ["ward-sell", "kittredge-sell", "rogers-sell", "rutschman-sell"]
};
...
function buildSessionDeck() {
  // Every run begins with one call from each major category. All 20 scenarios
  // are available in these four pools, including the four seller decisions.
  const selected = [
    randomItem(bucketIds.starter),
    randomItem(bucketIds.bullpen),
    randomItem(bucketIds.bat),
    randomItem(bucketIds.seller)
  ];

  // Fill the fifth and optional sixth slots from the entire remaining pool.
  // This makes every named scenario eligible for the initial deck instead of
  // hiding several players behind fallback-only conditions.
  const allIds = Object.values(bucketIds).flat();
  while (selected.length < MAX_CALLS) {
    const remaining = allIds.filter(id => !selected.includes(id));
    selected.push(randomItem(remaining));
  }
...
function calculateModel() {
  ...
  /*
    The baseline is a modestly above-.500 rest-of-season club before deadline
    moves. Accepted trades then add their marginal wins directly to the
    remaining schedule. This keeps standing pat risky while giving a strong
    buyer a credible path to October.
  */
  const baselinePct = 0.515;
  ...
  for (let i = 0; i < SIMULATIONS; i++) {
    ...
    */
    ...
    const cutoff = Math.round(83.5 + randomNormal() * 1.6);
    const madePlayoffs = finalWins >= cutoff;

    trials.push({ finalWins, cutoff, madePlayoffs });
    if (madePlayoffs) playoffCount++;
  }

  const averageWins = trials.reduce((sum, trial) => sum + trial.finalWins, 0) / trials.length;
  const playoffPct = Math.round(playoffCount / SIMULATIONS * 100);

  // Draw one complete trial so the displayed result uses the same cut line as
  // the playoff-probability model. An 83- or 84-win team can therefore make it
  // when that season's American League field is weak enough.
  const yourTrial = trials[Math.floor(Math.random() * trials.length)];
  const yourWins = yourTrial.finalWins;
  ...
  return {
    ...
    yourLosses,
    surprise,
    madePlayoffs: yourTrial.madePlayoffs,
    playoffCutoff: yourTrial.cutoff
  };
}
...
function outcomeFor(wins, madePlayoffs, cutoff) {
  if (madePlayoffs && wins >= 92) return "Won the AL East and reached October";
  if (madePlayoffs && wins >= 87) return "Claimed an American League wild-card berth";
  if (madePlayoffs) return `Survived the bubble and reached October (cut line: ${cutoff} wins)`;
  if (wins >= cutoff - 1) return `Missed October by one game (cut line: ${cutoff} wins)`;
  if (wins >= 82) return `Stayed alive until the final week but missed October (cut line: ${cutoff} wins)`;
  return `Fell short of the postseason (cut line: ${cutoff} wins)`;
}
  ...
  app.innerHTML = `
    <section class="results">
      <div class="result-hero">
        ...
        <p class="result-outcome">${made ? "✓" : "—"} ${outcomeFor(model.yourWins, model.madePlayoffs, model.playoffCutoff)}</p>
      ...
      <p class="method-note"><strong>Methodology:</strong> The live record comes from MLB’s standings feed when available. Baltimore begins with a .515 rest-of-season winning percentage. Each accepted trade applies a documented marginal-win adjustment, not the player’s full WAR. Every trial adds both ordinary game variance and a team-level performance shock so the current Orioles can run hot or cold. A variable playoff cut line is then applied. Trade offers are hypothetical.</p>
